...
def get_clubs(
    ...
    skip = (page - 1) * size
    items, total = FootballClubService.get_clubs(db, skip=skip, limit=size)
...
def search_clubs(
    ...
    skip = (page - 1) * size
    items, total = FootballClubService.search_clubs(db, query=q, skip=skip, limit=size)
    return {"items": items, "total": total, "page": page, "size": size}
...
def get_top_clubs(
    ...
    return FootballClubService.get_top_clubs(db, limit=limit)
